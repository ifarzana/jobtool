(function() {
    ...
    function ScheduleView($e) {
        ...
        this.refreshInterval = 60000; // 60000 milliseconds - 1 minute
    ...
    ScheduleView.prototype.build = function(data) {
        ...
        this.$container.append(this.$dayHeaders = this.createDayHeaders(data.days));
        ...
        this.$container.append(this.$tasks = this.createTasksOverlay(data.tasks));
    ...
    ScheduleView.prototype.createDayHeaders = function(days) {
        var $headers = $('<div>', {
        ...
        for (var i = 0; i < days.length; i++) {
            ...
            $headers.append($('<div>', {
                ...
                'html': '<strong>' + day.format('MMM DD') + '</strong>' /*Israt - this is where we put the header for time*/
            }));
        }
    ...
    ScheduleView.prototype.createHeaders = function(dates) {
        ...
        for (var i = 0; i < dates.length; i++) {
            var date = moment(dates[i], 'YYYY-MM-DD hh:mm'); /*Israt - this is to format the date range*/

            var header_class = ['schedule-header', 'tip-bottom'];

            $headers.append($('<div>', {
                'class': header_class.join(' '),
                'html': date.format('HH:mm') //or hA to have 7 am / 2 pm /*Israt - this is where we put the header for time*/
            }));
        }
    ...
    ScheduleView.prototype.createTasksOverlay = function(tasks) {
        var self = this;
        ...
        for (var i = 0; i < tasks.length; i++) {
            ...
            var start_offset = start.diff(this.start, 'hours') + 1; //when non workinghours is set put -7 for morning hours

            var day = 1;
            if(start_offset > 31) {
                day = Math.floor(start_offset / 24) + 1;
                // console.log('old = '+ start_offset);
                // console.log('DAY = '+ day);
            }
            ...
            if(day == 1){
                ...
                start_offset = start_offset - day*7 - (day-1)*4;

            }
            // console.log('newstart_offset = '+start_offset);


            var task_length = end.diff(start, 'hours') - 1;

            /*HERE YOU NEED TO DO THE CODING FOR THE TASKS LENGTH START AND END OFFSET*/
            var limit = start_offset +  task_length;
            console.log('task_length = '+ task_length);
            console.log('limit = '+ limit);

            if(start_offset + task_length > 13) {
                //task_length = limit - 13 -1;
            }

            if(end > this.end.endOf('day')) {
                end = this.end;
                task_length = end.diff(start, 'hours');
            }

            if(start < this.start) {
                start = this.start;
                start_offset = start.diff(this.start, 'hours');
                task_length = end.diff(start, 'hours');
            }
