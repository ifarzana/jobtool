...
if($edit_own_account == true) {
	$readonly_edit_own_account = 'readonly';
}else{
	$readonly_edit_own_account = null;
}
					...
					<div class="cipanel">
						...
						<div class="cipanel-content">
							...
							@if(Auth::user()->id == $result->id)
								<div class="alert alert-warning alert-list" role="alert">
									<i class="fa fa-exclamation-triangle"></i> All changes will be saved however if there are any changes detected to the 'Password' field you will be redirected to the authentication page !
								</div>
							@endif
							...
							<div class="row">
								<div class="col-md-12">
									...
									{!! Form::model($result, ['method' => 'POST', 'url' => UrlHelper::getUrl($controller, $action, $paginationData, array('id' => $result->id))]) !!}
									...
									<div class="row">
										<div class="col-md-6">
											<div class="form-group <?php if($errors->has('group_id')){ echo "has-error";} ?>">
												...
												{!! Form::select('group_id', [null => 'Please choose'] + $groups, null, ['class'=>'form-control', 'required', $readonly_edit_own_account]) !!}
											</div>
										</div>
										...
										<div class="col-md-6">
											<div class="form-group <?php if($errors->has('isActive')){ echo "has-error";} ?>">
												...
												{!! Form::select('isActive',[null => 'Please choose'] + [0 => 'No', 1 => 'Yes'], null,['class'=>'form-control', 'required', $readonly_edit_own_account]) !!}
											</div>
										</div>
									</div>
										...
										<div class="col-md-12">
											<div class="form-group <?php if($errors->has('colour')){ echo "has-error";} ?>">
												...
												<div class="input-group colour">
													...
													<span class="input-group-addon selected-colour" style="background-color:<?php echo isset($result->colour) ? $result->colour :'';?>;"><i class="fa fa-eyedropper"></i></span>
												</div>
											</div>
										</div>
									</div>
